...
enum HelpContent {
    ...
    static let configurationReference = HelpTopic(
        ...
        sections: [
            ...
            HelpSection(
                ...
                body: "ZWO cameras only. One tap sets RAW8, a small **Capture ROI**, and a safe starting exposure/gain for a specific bright target (Saturn, Jupiter, Mars, Venus, or the Moon), tuned around a modern ~2µm-pixel planetary camera (e.g. ASI678MC) behind a reference Maksutov 127mm/1500mm (f/11.8).",
                bullets: [
                    "The **Telescope** picker above the list scales the displayed/applied exposure for a different telescope's focal ratio (f/number) — a faster (lower f/number) scope needs less exposure for the same target brightness, a slower one more, the same relationship an ordinary camera's own exposure triangle uses for f/stop. It doesn't scale gain, and it can't account for a different camera's own sensitivity at all — these stay starting points to fine-tune against the live histogram, not exact values for your specific telescope/camera pairing.",
            ...
            HelpSection(
                ...
                bullets: [
                    "The **Telescope** picker at the top scales a planetary target's starting exposure for a different telescope's focal ratio — same as **Planetary Presets**' own picker, and the same `CameraManager.telescopeProfile` preference either one reads/writes. Changing it re-scales the currently-selected planetary target's exposure immediately; it has no effect on a deep-sky target's own starting point.",
            ...
            HelpSection(
                ...
                body: "GPU renderer only. An alternative to \"Reduce Drift\" above — instead of locking onto one star and shifting the whole frame by a single amount, this tracks an NxN grid of points spread across the frame, lets each one drift independently, then triangulates the mesh (each 4-point cell split into 2 triangles, the same primitive real-time rendering/games actually rasterize) and blends across each triangle with barycentric interpolation into a smooth, spatially-varying correction — not a 3D reconstruction of anything (there's no depth information to recover from a single 2D camera pointed at the night sky), just the standard unambiguous interpolation primitive for a 2D deformation mesh. Takes priority over \"Reduce Drift\" when both are on — they're two alternative techniques for the same job, not a combinable pair.",
                bullets: [
    ...
    static let projects = HelpTopic(
        ...
        sections: [
            ...
            HelpSection(
                heading: "Settings",
                body: "**Settings…** (⌘, or the Home page's own toolbar button) is the central place to review preferences: the Projects folder and Equipment folder locations (**Choose Folder…** picks anywhere else — takes effect the next time Skyformac launches, existing files aren't moved automatically), the renderer and Night Mode toggles already in the camera view's toolbar, an **AI (Ollama)** section (editable server URL, a model picker, and **Test Connection**, which checks both that the server answers and which models are actually installed, reporting exactly which one Ask AI to Plan/Describe would use), an **Astronomy Knowledge** folder (see below), and an **AI Skill: Suggest Next Session** text area — standing instructions (\"favor deep-sky over planetary,\" \"consider my dual-scope rig\") folded into every \"suggest my next session\" request, with a **Reset to Default**."
            ),
            HelpSection(
                heading: "Astronomy Knowledge",
                body: "A small model like `qwen3:8b` doesn't reliably know which Messier objects are best in which season, or that Venus is only ever visible near dawn/dusk — the **Astronomy Knowledge** folder (Settings) is a set of plain `.md` reference files folded into every AI request to ground it in real facts instead of guessed ones. Add, edit, or delete any `.md` file directly in Finder (**Reveal in Finder**, in Settings, opens straight to it) to extend or correct it — your own local site notes, a correction to a fact, anything. **Restore Default Content** resets just the two shipped default files back to their original text, leaving anything else you've added completely alone. This still isn't live position calculation — the app doesn't compute real-time altitude/azimuth, so \"what's up right now\" from your exact location still benefits from checking a planetarium app or almanac, especially for fast-moving planets."
            ),
            HelpSection(
