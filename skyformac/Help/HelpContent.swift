    ...
    static let configurationReference = HelpTopic(
        ...
        sections: [
            ...
            HelpSection(
                ...
                body: "A running average of incoming frames. By default no star alignment — this assumes a tracked, stationary mount, the same scoping SharpCap's basic live-stack mode uses. See **Reduce Drift** below for basic alignment. Exporting FITS/PNG/TIFF (Export, above) while this is on exports the actual stacked average, on both the GPU and CPU render paths — not just whatever the latest single frame happened to be. Works for an iPhone/webcam source too — it always accumulates on the CPU running-average regardless of the GPU/CPU toggle above, since the GPU accumulator only supports a ZWO camera's mono sensor data; the live preview still renders on the GPU either way. \"Save Stacked Image…\" saves the stack exactly as it looks right now as a PNG, without needing the Export section below."
            ),
            HelpSection(
